                        ...
                        <div className="bg-white shadow rounded-lg p-6">
                                ...
                                <div className="mb-4 flex items-center bg-gray-50 p-3 rounded border border-gray-200">
                                    <div className="flex-1">
                                        ...
                                        <div className="text-sm font-mono text-gray-700 mt-1 flex items-center">
                                            ...
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium uppercase ${keyStatus.status === 'Inactive' ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800'}`}>
                                                {keyStatus.status}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div className="flex items-center justify-between mt-4">
                                <div className="text-sm text-gray-500">
                                    {keyStatus?.status === 'Inactive'
                                        ? "Generate your initial API key to start."
                                        : "Rotate key if compromised."}
                                </div>
                                <button
                                    onClick={rotateKey}
                                    disabled={loading}
                                    className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 ${keyStatus?.status === 'Inactive'
                                        ? 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500'
                                        : 'bg-red-600 hover:bg-red-700 focus:ring-red-500'
                                        }`}
                                >
                                    {loading ? '...' : (keyStatus?.status === 'Inactive' ? 'Generate API Key' : 'Rotate Key')}
                                </button>
                            </div>
                            {newKey && (
